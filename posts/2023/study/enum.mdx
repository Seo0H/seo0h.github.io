...
프로젝트를 만들어 가다 보면, 고정된 값들을 보관할 변수가 필요합니다. 이런 값을 '상수' 라고 하는데요, 변하지 않는 값을 상수로 선언해 관리합니다.

JavaScript 에서 상수 객체를 만들기 위해선 Object 객체를 이용해 만드는 것 밖에 없었지만, TypeScript 에서는 다른 언어들에서 지원하는 기능인 **열거형(Enum)** 을 지원하고, 또한 **const assertions** 을 통해서 읽기 전용 객체를 만드는 것도 가능합니다.

이런 선택지들 중 **TypeScript에서 상수 관리 객체를 선언하는 방법**을 살펴보고, 각 **장단점을 정리** 해보고자 합니다.
...
<details>
   <summary>**❔`const` assertions (const 주장) 이 뭔가요?** </summary>
       <div markdown="1">
       <br />
        - [const assertions](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-4.html#const-assertions)

       TypeScript 3.4에 추가된 리터럴 표현식(=기호 표현식) 구성으로, 유형 이름 대신 const를 사용하는 type 입니다.
